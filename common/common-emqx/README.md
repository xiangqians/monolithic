...
MQTT（Message Queuing Telemetry Transport）是一种轻量级的消息传输协议，特别适用于物联网设备之间的通信。
...
![Mqtt publish subscribe](doc/image/mqtt_publish_subscribe.png)
...
![MQTT Share Subscribe](doc/image/mqtt_share_subscribe.png)
...
这取决于 MQTT broker 的实现，例如 EMQX 会为每个匹配的订阅发送消息。但是用户可以使用 MQTT 5.0 中的订阅标识符来区分消息来源，然后在客户端中根据订阅标识符来处理这类重复的消息。
