...
//        ErrorController,
    ...
    public Object beforeBodyWrite(Object body, MethodParameter returnType, MediaType selectedContentType, Class<? extends HttpMessageConverter<?>> selectedConverterType, ServerHttpRequest request, ServerHttpResponse response) {
        try {
            log(((ServletServerHttpRequest) request).getServletRequest(), body instanceof Result ? (Result) body : null);
        } catch (Throwable t) {
            log.error("", t);
        }
        return body;
    ...
    public void log(HttpServletRequest request, Result response) throws IOException {
        // 不记录日志接口日志
...
//        log.setBody(body);
        ...
        log.setCode(Optional.ofNullable(response).map(Result::getCode).orElse(null));
